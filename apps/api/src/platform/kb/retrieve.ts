...
export async function retrieve(query: string, opts: RetrievalOptions = {}): Promise<RetrievalHit[]> {
  ...
  const [vec] = await getEmbedding().embed([q]);
  ...
  const vectorRows = await sql<any[]>`
  ...
  const keywordRows = await sql<any[]>`
    SELECT c.id, c.document_id, d.title AS document_title, d.uri, d.authority, c.content, c.context_header,
           ts_rank(c.tsv, plainto_tsquery('simple', ${q})) + similarity(c.content, ${q}) AS kscore
    FROM kb_chunks c JOIN kb_documents d ON d.id = c.document_id
    WHERE (c.tsv @@ plainto_tsquery('simple', ${q}) OR c.content % ${q}) ${tagFilter} ${wfFilter} ${visFilter} ${collFilter} ${effFilter}
