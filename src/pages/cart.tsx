...
export default function Cart() {
  ...
  const handleBuyNow = async () => {
    const result = await runCheckout(items);
    if (result) setOrderSuccess({ order_number: result.orderNumber, total: result.total });
  };
          ...
          <div className="grid lg:grid-cols-12 gap-12 lg:gap-16 items-start">
            <div className="lg:col-span-8 space-y-4">
              <div className="flex items-center justify-between pb-4 border-b border-black/10 font-inter text-xs text-black/40 uppercase tracking-wider font-medium">
                <span>Product</span>
                <button onClick={clearCart} className="hover:text-black transition-colors">
                  Clear Cart
                </button>
              </div>
                ...
                >
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <span className="font-dm text-lg text-black font-normal">₹{item.price.toFixed(0)}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-6 pt-4 sm:pt-0 border-t sm:border-t-0 border-black/5">
                    <div className="flex items-center border border-black/15 rounded-xl bg-[#FAF9F5] p-1">
                      ...
                      <span className="w-10 text-center font-inter text-sm font-medium">{item.quantity}</span>
                      <button
                        onClick={() => updateQuantity(item.id, item.quantity + 1)}
                        className="w-8 h-8 rounded-lg flex items-center justify-center text-black/60 hover:text-black hover:bg-black/5 transition-colors"
                      >
            ...
            <div className="lg:col-span-4">
              <div className="bg-[#FEFDF9] rounded-3xl p-8 border border-black/5 shadow-xl sticky top-28">
                <h3 className="font-dm font-normal text-2xl text-black tracking-[-0.03em] mb-6">Order Summary</h3>
                ...
                <div className="flex justify-between items-baseline mb-6">
                  ...
                  <span className="font-dm text-3xl text-black font-normal tracking-[-0.03em]">₹{subtotal.toFixed(0)}</span>
                </div>
