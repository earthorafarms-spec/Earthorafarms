import type {
  ...
  ConversationMessage,
  CartSnapshotLine,
  CheckoutFieldSnapshot,
} from '../../voice-service/src/conversation/state.js';
import {
  WHATSAPP_MENU,
...
import { enforceOutputPolicy } from '../../voice-service/src/conversation/output-policy.js';
import {
  detectLanguage,
  requestedLanguage,
  buildLanguageInstruction,
  type SupportedLanguage,
} from '../../voice-service/src/conversation/language.js';
import { allTools, toolsByName } from '../../voice-service/src/tools/index.js';
import {
  isCheckoutReady,
  isIndiaAffirmative,
...
import { OpenAiLLMAdapter } from '../../voice-service/src/adapters/openai.js';
import { SarvamLLMAdapter } from '../../voice-service/src/adapters/sarvam.js';
import type { ToolDefinition } from '../../voice-service/src/tools/types.js';
import type { LLMTurnResult } from '../../voice-service/src/adapters/types.js';
import { config } from '../../voice-service/src/config.js';

// ── Lazy-instantiated LLM adapters (WhatsApp text only, zero audio/STT/TTS coupling) ──
...
export async function chatWithRouting(
  ...
  if (config.LLM_PROVIDER !== 'auto') {
    return (config.LLM_PROVIDER === 'sarvam' ? getSarvam() : getOpenAi()).chatWithTools(messages, tools);
  }
  ...
  if (language !== 'en') {
    try {
      ...
      console.warn(`[whatsapp-llm] Sarvam call failed, falling back to OpenAI: ${(err as Error).message}`);
    }
  }
...
export function nextWhatsAppCheckoutQuestion(state: ConversationState, _includeTimeoutNotice = true): string | null {
...
export function buildWhatsAppMenuReply(language: ConversationState['currentLanguage']): string {
...
export function buildViewCartCard(state: ConversationState): WhatsAppProductCard {
...
export function buildNumberedCatalogReply(
  catalog: unknown,
  language: ConversationState['currentLanguage'],
  invalidSelection = false,
): string {
...
export function buildWhatsAppProductCard(details: LiveProductDetails, fallbackName: string): WhatsAppProductCard | null {
...
export function lastAssistantReply(messages: ConversationMessage[]): string | null {
...
export function liveCatalogProducts(catalog: unknown): LiveCatalogProduct[] {
...
function directCartReply(state: ConversationState): string {
  ...
  const lines = cart.map((line) => {
    const quantity = QUANTITY_WORDS[language][line.quantity] ?? String(line.quantity);
...
function buildCheckoutTurnInstruction(state: ConversationState): string | null {
  if (state.cart.length === 0) return null;

  const nextMissing = CHECKOUT_FIELDS.find((field) => !state.checkoutFields[field]);
  if (!nextMissing) {
  ...
  return (
    `CHECKOUT FIELD EXPECTED NOW: ${nextMissing}. Unless the customer clearly changes the topic, treat their ` +
...
export async function processWhatsAppTurn(
  ...
  const pendingProduct = state.whatsAppProductContext?.awaitingQuantity
  ...
  // Main menu
  if (shouldShowWhatsAppMenu(userText, state)) {
    delete state.whatsAppProductContext;
    delete state.awaitingCartRemoval;
    const replyText = buildWhatsAppMenuReply(state.currentLanguage);
    state.messages.push({ role: 'assistant', content: replyText });
  ...
  // View Cart selection
  if (isViewCartSelection(userText, lastAssistantReply(state.messages), state)) {
    delete state.whatsAppProductContext;
    delete state.awaitingCartRemoval;
    const cartTool = toolsByName.get_cart;
    const cartResult = cartTool
  ...
  if (isRemoveItemSelection(userText, lastAssistantReply(state.messages), state)) {
    delete state.whatsAppProductContext;
    if (state.cart.length === 0) {
      delete state.awaitingCartRemoval;
      const cartCard = buildViewCartCard(state);
  ...
  // Continue shopping selection
  if (isContinueShoppingSelection(userText, lastAssistantReply(state.messages), state)) {
    delete state.whatsAppProductContext;
    delete state.awaitingCartRemoval;
  ...
  // Policy selections
  if (isPoliciesMenuSelection(userText, state.messages, state)) {
    delete state.whatsAppProductContext;
    delete state.awaitingCartRemoval;
  ...
  if (isShippingPolicySelection(userText, state.messages, state)) {
    delete state.whatsAppProductContext;
    delete state.awaitingCartRemoval;
  ...
  if (isReturnPolicySelection(userText, state.messages, state)) {
    delete state.whatsAppProductContext;
    delete state.awaitingCartRemoval;
  ...
  // Active checkout question field answering
  const previousReplyForCheckout = lastAssistantReply(state.messages);
  const activeCheckoutField = state.cart.length > 0
  ...
  if (activeCheckoutField !== null) {
    ...
    if (!isGreeting && !isDigitOnName) {
      ...
      if (activeCheckoutField === 'name') {
        toolResult = await toolsByName.set_checkout_field.handler({ field: 'name', value: trimmed }, toolContext);
      } else if (activeCheckoutField === 'email') {
        toolResult = await toolsByName.set_checkout_field.handler({ field: 'email', value: trimmed }, toolContext);
      } else if (activeCheckoutField === 'phone') {
        toolResult = await toolsByName.set_checkout_field.handler({ field: 'phone', value: trimmed }, toolContext);
      } else if (activeCheckoutField === 'address') {
        toolResult = await toolsByName.set_checkout_field.handler({ field: 'address', value: trimmed }, toolContext);
      } else if (activeCheckoutField === 'city_state') {
        const parsed = parseCityAndStateInput(trimmed);
        if (parsed.state) {
          toolResult = await toolsByName.set_delivery_location.handler({ city: parsed.city, state: parsed.state }, toolContext);
        } else {
          toolResult = await toolsByName.set_checkout_field.handler({ field: 'city', value: parsed.city }, toolContext);
        }
      } else if (activeCheckoutField === 'state') {
        toolResult = await toolsByName.set_checkout_field.handler({ field: 'state', value: trimmed }, toolContext);
      } else if (activeCheckoutField === 'postalCode') {
        toolResult = await toolsByName.set_checkout_field.handler({ field: 'postalCode', value: trimmed }, toolContext);
      } else if (activeCheckoutField === 'country') {
        if (isIndiaAffirmative(trimmed)) {
          toolResult = await toolsByName.set_checkout_field.handler({ field: 'country', value: 'India' }, toolContext);
          ...
          toolResult = await toolsByName.set_checkout_field.handler({ field: 'country', value: trimmed }, toolContext);
        }
        ...
        if (isBareAffirmative) {
          ...
          state.messages.push({ role: 'assistant', content: basePrompt });
          return { state, replyText: basePrompt, policyViolations: [], outboundActions };
        }
        ...
        toolResult = await toolsByName.set_checkout_field.handler({ field: 'gst', value: gstValue }, toolContext);
      }
  ...
  if (listTool) {
    ...
    if (!productAction && shouldAnswerCatalogDirectly(userText)) {
      ...
      if (directReply) {
        state.messages.push({ role: 'assistant', content: directReply });
        return { state, replyText: directReply, policyViolations: [], outboundActions };
      }
    }
    ...
    const selectedProduct = actionProduct ?? (selectedProductNumber !== null
      ? numberedProduct
    ...
    if (selectedProduct) {
      ...
      if (detailsTool) {
        ...
        if (!productAction && PRODUCT_PRICE_PATTERN.test(userText) && typeof livePrice === 'number') {
          ...
          state.messages.push({ role: 'assistant', content: groundedReply });
          return { state, replyText: groundedReply, policyViolations: [], productImage, outboundActions };
        }
        ...
        if (liveDetails && liveDetails.found) {
        ...
        // Native 2-button Benefits & Dosage UX response
        if (productAction && actionProduct) {
          state.whatsAppProductContext = {
            productId: actionProduct.id,
            productName: actionProduct.name,
          ...
          if (productAction.action === 'benefits' || productAction.action === 'dosage') {
  ...
  // Fallback to LLM loop
  const basePrompt = WHATSAPP_SYSTEM_PROMPT;
  ...
  for (let iteration = 0; iteration < MAX_TOOL_LOOP_ITERATIONS; iteration++) {
    ...
    if (result.kind === 'tool_calls') {
      ...
      for (const call of result.calls) {
        ...
        const resJson = JSON.stringify(resultObj);
        iterationResults.push({ name: call.name, value: resultObj });
        state.currentTurnFacts.push({ toolName: call.name, resultJson: resJson });
        state.messages.push({ role: 'tool', content: resJson, toolCallId: call.id, toolName: call.name });
      }
      ...
      if (fieldSaved && isCheckoutReady(state) && !state.currentTurnFacts.some((fact) => fact.toolName === 'create_verification_link')) {
        ...
        const resJson = JSON.stringify(delivery);
        state.currentTurnFacts.push({ toolName: 'create_verification_link', resultJson: resJson });
        state.messages.push({ role: 'tool', toolName: 'create_verification_link', toolCallId: callId, content: resJson });
      }
      ...
      if (successfulCartMutation) {
        ...
        const replyText = formatWhatsAppReply(exactReply);
        state.messages.push({ role: 'assistant', content: replyText });
    ...
    if (productImage && looksLikeSingleProductRecordDump(result.content) && productStyleRegenerateAttempts < 1) {
    ...
    if (policyResult.action === 'regenerate' && regenerateAttempts < MAX_POLICY_REGENERATE_ATTEMPTS) {
      regenerateAttempts++;
      // eslint-disable-next-line no-console
      console.warn('[whatsapp-output-policy] regenerating turn:', policyResult.violations);
      transientCorrection = { role: 'system', content: `[Correction] ${policyResult.instruction}` };
      continue;
    }

    const finalText = formatWhatsAppReply(policyResult.text);
    state.messages.push({ role: 'assistant', content: finalText });
    return { state, replyText: finalText, policyViolations: policyResult.violations, productImage, productCard, outboundActions };
  }

  const fallback = 'I had trouble processing that. Please choose from the main menu or ask your question again.';
