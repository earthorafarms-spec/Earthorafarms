...
async def run_case(agent, bridge, chat, *, question, language, inference, expected_conflict=False):
    bridge.case = language
    ...
    calls = bridge.calls[calls_start:]
    ...
    events = agent.events.items[events_start:]
    ...
    checks = {
        ...
        "expected_inference_path": inference_calls == 0 if expected_conflict else inference_calls > 0,
    ...
    evidence = [{"title": str(entry.get("title", "")), "text_sha256": hashlib.sha256(str(entry.get("text", "")).encode()).hexdigest()} for entry in knowledge]
    return {"language": language, "scenario": "usage_conflict" if expected_conflict else "ingredients", "question": question, "reply": answer, "rounds": rounds,
            ...
            "evidence": evidence, "checks": checks, "passed": all(checks.values())}
...
async def run(args):
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(20, connect=5), follow_redirects=False) as client:
            ...
            cases = []
            for language, scenario in (("en", "ingredients"), ("gu", "ingredients"), ("en", "usage_conflict"), ("gu", "usage_conflict")):
                ...
                with patch.object(model, "chat", wraps=model.chat) as inference:
                    case = await run_case(agent, bridge, agent.chat_ctx.copy(), question=question, language=language, inference=inference, expected_conflict=scenario == "usage_conflict")
