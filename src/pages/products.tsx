...
import { Star, ShoppingCart, Zap, Truck, Heart, X, ChevronDown, User, Calendar, ThumbsUp, Eye } from "lucide-react";
...
import capsulesHover from "@assets/generated_images/product_capsules_2.jpg";
import powderMain from "@assets/generated_images/product_powder.jpg";
import powderHover from "@assets/generated_images/product_powder_2.jpg";
import tabletsMain from "@assets/generated_images/product_tablets.jpg";
import tabletsHover from "@assets/generated_images/product_tablets_2.jpg";
import amlaMain from "@assets/generated_images/hero_leaves.jpg";
import amlaHover from "@assets/generated_images/hero_leaves_2.jpg";
...
const products: Product[] = [
  {
    id: "capsules",
    name: "Earthora Organic Moringa Capsules",
    mrp: 999,
    price: 699,
    discount: 30,
    rating: 4.6,
    reviewCount: 214,
    tag: "500mg · 90 Capsules",
    imageMain: capsulesMain,
    imageHover: capsulesHover,
    badge: "Best Seller",
    delivery: "FREE delivery Tomorrow, Jul 14",
    stock: "In Stock",
    highlights: [
      "500 mg organic moringa leaf per capsule",
      "90 vegetable capsules — 3 month supply",
      "No fillers, binders, or flow agents",
      "Third-party tested for purity & potency",
    ],
    description: "Our premium moringa capsules deliver the full nutritional profile of fresh moringa leaves in a convenient daily format. Sourced from our family farm.",
  },
  {
    id: "powder",
    name: "Earthora Pure Moringa Leaf Powder",
    mrp: 849,
    price: 599,
    discount: 29,
    rating: 4.7,
    reviewCount: 187,
    tag: "8 oz · Resealable Pouch",
    imageMain: powderMain,
    imageHover: powderHover,
    badge: "Most Popular",
    delivery: "FREE delivery Tomorrow, Jul 14",
    stock: "In Stock",
    highlights: [
      "100% pure shade-dried moringa leaf powder",
      "Stone-ground at low temperature",
      "Smooth texture — blends instantly",
      "8 oz resealable stand-up pouch",
    ],
    description: "Harvested by hand and stone-ground to preserve nutrients. Perfect for smoothies, teas, and cooking. No additives, ever.",
  },
  {
    id: "tablets",
    name: "Earthora Pressed Moringa Tablets",
    mrp: 1099,
    price: 799,
    discount: 27,
    rating: 4.8,
    reviewCount: 156,
    tag: "500mg · 120 Tablets",
    imageMain: tabletsMain,
    imageHover: tabletsHover,
    badge: "Value Pack",
    delivery: "FREE delivery Tomorrow, Jul 14",
    stock: "In Stock",
    highlights: [
      "500 mg pressed moringa per tablet",
      "120 tablets — 4 month supply",
      "Zero binders, fillers, or coatings",
      "Biodegradable, plastic-free packaging",
    ],
    description: "Our pressed moringa tablets contain nothing but the leaf — no magnesium stearate, no silica. High-pressure pressed for natural binding.",
  },
  {
    id: "amla",
    name: "Earthora Organic Amla Powder",
    mrp: 649,
    price: 449,
    discount: 31,
    rating: 4.7,
    ...
    tag: "8 oz · Resealable Pouch",
    imageMain: amlaMain,
    imageHover: amlaHover,
    badge: "New",
    delivery: "FREE delivery Tomorrow, Jul 14",
    stock: "In Stock",
    highlights: [
      "100% pure wild-harvested amla fruit powder",
      "Cold-pressed to preserve natural vitamin C",
      "Supports immunity, hair health & digestion",
      "8 oz resealable stand-up pouch",
    ],
    description: "Sourced from wild-grown amla (Indian gooseberry) trees, our powder is cold-pressed to retain the highest natural vitamin C content of any fruit. A potent Ayurvedic superfood.",
  },
];
...
export default function Products() {
  ...
  const handleBuyNow = (p: Product) => {
    ...
    toast({
      title: "Order placed!",
      description: `${p.name} will be delivered ${p.delivery.toLowerCase()}.`,
    });
  };
  ...
  useEffect(() => {
    ...
    setModalImage(selectedProduct.imageMain);
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col bg-background text-foreground selection:bg-primary/20">
        ...
        <div className="container mx-auto px-6 max-w-7xl">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-8 pb-6 border-b border-border/50">
            <p className="text-sm text-foreground/50">
              <span className="text-foreground font-medium">{sortedProducts.length}</span> products
            </p>
                    ...
                    >
                      {sortOptions.map((opt) => (
                        <button
                          ...
                          className={`w-full flex items-center gap-2 px-4 py-2.5 text-sm text-left transition-colors hover:bg-muted ${
                            sortBy === opt ? "text-foreground font-medium" : "text-foreground/60"
                          }`}
                        >
                          <span className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
                            sortBy === opt ? "border-primary" : "border-border"
                          }`}>
              ...
              >
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-foreground/40">({p.reviewCount})</span>
                  </div>

                  <div className="flex items-baseline gap-2 mb-3">
                    <span className="text-xl font-bold text-foreground">₹{p.price.toFixed(2)}</span>
                    <span className="text-sm text-foreground/30 line-through">₹{p.mrp.toFixed(2)}</span>
                    <span className="text-xs font-semibold text-accent">{p.discount}% off</span>
                  </div>
      ...
      <AnimatePresence>
        {selectedProduct && (
          <motion.div
            key={`backdrop-${selectedProduct.id}`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
        ...
        {selectedProduct && (
          ...
          >
            <div className="flex items-center justify-between px-6 md:px-10 h-16 border-b border-border/30 shrink-0">
              <h2 className="text-sm font-serif text-foreground truncate">
              ...
              <div className="flex-1 overflow-y-auto">
                <div className="flex flex-col lg:flex-row">
                  <div className="lg:w-[45%] bg-[#fafaf8] p-8 lg:p-12 flex flex-col items-center justify-start border-b lg:border-b-0 lg:border-r border-border/20">
                    <div className="w-full max-w-md aspect-square relative flex items-center justify-center">
                      <img
                        src={modalImage || selectedProduct.imageMain}
                        alt={selectedProduct.name}
                        className="w-full h-full object-contain drop-shadow-sm"
                      />
                    </div>
                    <div className="flex gap-3 mt-6">
                      {[selectedProduct.imageMain, selectedProduct.imageHover].map((src, i) => (
                        <button
                          key={i}
                          onClick={() => setModalImage(src)}
                          className={`w-16 h-16 rounded-xl border-2 overflow-hidden bg-white p-2 hover:border-primary/40 transition-colors ${
                            modalImage === src ? "border-primary" : "border-border/40"
                          }`}
                        >
                  ...
                  <div className="lg:w-[55%] p-6 md:p-10">
                    <div className="max-w-xl">
                      ...
                      <h1 className="text-2xl md:text-3xl font-serif text-foreground leading-tight mb-2">
                        {selectedProduct.name}
                      </h1>
                      ...
                      <div className="flex items-center gap-3 mb-6 pb-6 border-b border-border/20">
                        <div className="flex items-center gap-1.5">
                          <span className="text-sm font-bold text-foreground bg-accent/20 px-2 py-0.5 rounded-md">
                          ...
                          <div className="flex gap-0.5">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <Star
                                key={i}
                                className={`w-4 h-4 ${i < Math.round(selectedProduct.rating) ? "fill-accent text-accent" : "text-border"}`}
                                strokeWidth={1.5}
                              />
                            ))}
                          </div>
                        </div>
                        <span className="text-sm text-foreground/40">{selectedProduct.reviewCount} ratings</span>
                      ...
                      <div className="flex items-baseline gap-3 mb-6">
                        ...
                        <span className="text-lg text-foreground/30 line-through">₹{selectedProduct.mrp.toFixed(2)}</span>
                ...
                <div className="border-t border-border/20 px-6 md:px-12 py-10 bg-secondary/10">
                  <div className="max-w-4xl mx-auto">
                    ...
                    <div className="bg-card rounded-2xl p-6 md:p-8 border border-border/30 mb-8">
                      <div className="flex flex-col md:flex-row gap-6 md:items-center">
                        <div className="text-center md:text-left">
                          <div className="text-5xl font-bold text-foreground tracking-tight">{selectedProduct.rating}</div>
                          ...
                          <p className="text-sm text-foreground/40 mt-1">{selectedProduct.reviewCount} ratings</p>
                        </div>
                        <div className="flex-1 space-y-1.5">
                          {[5, 4, 3, 2, 1].map((star) => {
                            const pct = selectedProduct.id === "capsules" ? [60, 25, 10, 3, 2]
                              : selectedProduct.id === "powder" ? [55, 30, 10, 3, 2]
                              : selectedProduct.id === "tablets" ? [65, 22, 8, 3, 2]
                              : [50, 30, 12, 5, 3];
                            return (
                              <div key={star} className="flex items-center gap-2">
                                <span className="text-xs text-foreground/40 w-3">{star}</span>
                                <Star className={`w-3 h-3 ${star <= Math.round(selectedProduct.rating) ? "fill-accent text-accent" : "text-border"}`} strokeWidth={1.5} />
                                <div className="flex-1 h-2 rounded-full bg-border/50 overflow-hidden">
                                  <div className="h-full rounded-full bg-accent/70" style={{ width: `${pct[5 - star]}%` }} />
                                </div>
                                <span className="text-xs text-foreground/30 w-8 text-right">{pct[5 - star]}%</span>
                              </div>
                            );
                          })}
                        </div>
                      ...
                      <div className="lg:w-80">
                        ...
                        <div className="bg-card rounded-2xl p-5 border border-border/30 space-y-4">
                          ...
                          <Button className="w-full h-11 text-sm" onClick={() => handleSubmitReview(selectedProduct.id)} disabled={!reviewName.trim() || !reviewComment.trim()}>
                            <ThumbsUp className="w-4 h-4 mr-1.5" strokeWidth={1.5} />
                            Submit Review
                          </Button>
                        </div>
                      </div>
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-0.5">
                                      <div className="flex gap-0.5">
                                        {Array.from({ length: 5 }).map((_, i) => (
                                          <Star key={i} className={`w-3 h-3 ${i < r.rating ? "fill-accent text-accent" : "text-border"}`} strokeWidth={1.5} />
                                        ))}
                                      </div>
                ...
                <div className="border-t border-border/20 px-6 md:px-12 py-10">
                  <div className="max-w-5xl mx-auto">
                    <div className="flex items-center justify-between mb-6">
                      <h3 className="text-lg font-serif text-foreground">You may also like</h3>
                      <div className="flex gap-1">
                        {products.filter((p) => p.id !== selectedProduct.id).length > 4 && (
                          <button className="p-2 rounded-full border border-border/40 hover:bg-muted transition-colors">
                            <svg className="w-4 h-4 text-foreground/60" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
                          </button>
                        )}
                      </div>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
                      {products.filter((p) => p.id !== selectedProduct.id).slice(0, 4).map((similar) => (
                        <button
                          key={similar.id}
                        ...
                        >
                          <div className="aspect-square bg-white p-5 flex items-center justify-center relative">
                            <img
                              src={similar.imageMain}
                              alt={similar.name}
                              className="w-full h-full object-contain transition-transform duration-500 group-hover:scale-105"
                            />
                            {similar.badge && (
                              <span className="absolute top-2.5 left-2.5 px-2 py-0.5 text-[10px] font-bold bg-primary text-primary-foreground rounded-md">
                                {similar.badge}
                              </span>
                            )}
                          ...
                          <div className="p-3.5">
                            ...
                            <div className="flex items-center gap-1 mb-1.5">
                              ...
                              <span className="text-[10px] text-foreground/30">({similar.reviewCount})</span>
                            </div>
                            <div className="flex items-baseline gap-1.5">
                              <span className="text-sm font-bold text-foreground">₹{similar.price.toFixed(2)}</span>
                              <span className="text-[10px] text-foreground/30 line-through">₹{similar.mrp.toFixed(2)}</span>
                            </div>
